...
namespace Graxei.Search.Tests.Shards
{
    [TestFixture]
    public class ShardsTest : PhysicalTestCase
    {
        ...
        public void InternalSharding()
        {
            ...
            using(IndexReader reader = IndexReader.Open(new FileInfo(BaseIndexDir.FullName + "\\Animal00"),true))
            {
                int num = reader.NumDocs();
                Assert.AreEqual(1, num);
            }

            using(IndexReader reader = IndexReader.Open(new FileInfo(BaseIndexDir.FullName + "\\Animal.1"),true))
            {
                int num = reader.NumDocs();
                Assert.AreEqual(1, num);
            }
