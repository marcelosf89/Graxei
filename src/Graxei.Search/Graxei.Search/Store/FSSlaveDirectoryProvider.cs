    ...
    public class FSSlaveDirectoryProvider : IDirectoryProvider
    {
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                directory1 = FSDirectory.Open(subDir.FullName);
                create = !IndexReader.IndexExists(directory1);
                if (create)
                {
                    log.DebugFormat("Initialize index: '{0}'", subDir.FullName);
                    using (IndexWriter iw1 = new IndexWriter(directory1, new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_30),create,IndexWriter.MaxFieldLength.UNLIMITED))
                    {
                    }
                }

                subDir = new DirectoryInfo(Path.Combine(indexName, "2"));
                directory2 = FSDirectory.Open(subDir.FullName);
                create = !IndexReader.IndexExists(directory2);                 
                if (create)
                {
                    log.DebugFormat("Initialize index: '{0}'", subDir.FullName);
                    using (IndexWriter iw2 = new IndexWriter(directory2, new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_30), create, IndexWriter.MaxFieldLength.UNLIMITED))
                    {
                    }
                }
