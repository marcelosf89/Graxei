...
namespace Graxei.Search.Query
{
    ...
    public class FullTextQueryImpl : QueryImpl, IFullTextQuery
    {
            ...
            using (new SessionIdLoggingContext(Session.SessionId))
            {
                ...
                try
                {
                    ...
                    foreach (ScoreDoc doc in hits.ScoreDocs)
                    {
                        ...
                        infos.Add(extractor.Extract(doc, searcher));
                    }
        ...
        public override void List(IList list)
        {
            using (new SessionIdLoggingContext(Session.SessionId))
            {
                ...
                try
                {
                    ...
                    SetResultSize(hits);
                    int first = First();
                    int size = hits.ScoreDocs.Length - first + 1;
                    if (size <= 0)
                    {
                        return;
                    }
        ...
        private TopDocs GetHits(Searcher searcher)
        {
            using (new SessionIdLoggingContext(Session.SessionId))
            {
                LogQuery();
                ...
                BuildFilters();
                TopDocs hits = searcher.Search(query, this.filter, searcher.MaxDoc);
        ...
        private int Max(int first, TopDocs hits)
        {
            ...
            {
                return hits.ScoreDocs.Length - 1;
            }

            if (Selection.MaxRows + first < hits.ScoreDocs.Length)
            {
