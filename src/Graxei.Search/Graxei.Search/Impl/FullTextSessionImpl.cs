...
using Graxei.Search.Query;
using Graxei.Search.Util;
...
namespace Graxei.Search.Impl
{
    public class FullTextSessionImpl : IFullTextSession
    {
        ...
        public IFullTextQuery CreateFullTextQueryAllField<TEntity>(string queryString)
        {
            List<string> ls = new List<string>();
            ls = GetPropertFields(typeof(TEntity));

            using (new SessionIdLoggingContext(sessionImplementor.SessionId))
            {
                MultiFieldQueryParser queryParser = new MultiFieldQueryParser(Lucene.Net.Util.Version.LUCENE_30, ls.ToArray(), new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_30));
                Lucene.Net.Search.Query query = queryParser.Parse(queryString);
                return CreateFullTextQuery(query, typeof (TEntity));
            }
        }

        private List<string> GetPropertFields(System.Type type, String fieldName = null)
        {
            List<string> ls = new List<string>();
            PropertyInfo[] a = type.GetProperties();

            String fieldNameAgregate = "";
            if (!String.IsNullOrEmpty(fieldName))
            {
                fieldNameAgregate = fieldName + ".";
            }

            foreach (PropertyInfo prop in a)
            {
                IEnumerable<FieldAttribute> fa = prop.GetCustomAttributes<FieldAttribute>(true);
                if (fa.Any())
                {
                    ls.Add(fieldNameAgregate+ prop.Name);
                }

                IEnumerable<IndexedEmbeddedAttribute> fa2 = prop.GetCustomAttributes<IndexedEmbeddedAttribute>(true);
                if (fa2.Any())
                {
                    ls.AddRange(GetPropertFields(prop.PropertyType, fieldNameAgregate + prop.Name));
                }
            }
